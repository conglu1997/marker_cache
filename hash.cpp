...
size_t default_hash_function::operator()(object const& o) const {
    // FIXME: fall back to a generic universal hash function (e.g., HMAC/MD5)
    // for too large objects. Current max is 36 bytes
    if (o.size() > max_obj_size) throw std::runtime_error("object too large");
...
std::vector<digest> default_hasher::operator()(object const& o) const {
    ...
    for (auto i = 0; i < fns_.size(); ++i) d[i] = fns_[i](o);
    return d;
}
...
std::vector<digest> double_hasher::operator()(object const& o) const {
    ...
    for (auto i = 0; i < d.size(); ++i) d[i] = d1 + i * d2;
    return d;
}
...
hasher make_hasher(size_t k, size_t seed, bool double_hashing) {
    ...
    if (double_hashing) {
        ...
        for (auto i = 0; i < k; ++i) fns[i] = default_hash_function(prng());
